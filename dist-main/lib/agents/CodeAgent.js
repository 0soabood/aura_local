...
const SYSTEM_PROMPT = 'You are the Code Agent — a senior software engineer with deep expertise in TypeScript, ' +
 ...
 */
class CodeAgent extends types_1.BaseAgent {
    constructor() {
        super(...arguments);
     ...
     */
    lastCodeAgentErrorMessage(events) {
        const last = events.at(-1);
        if (!last || last.event_type !== 'execution_error')
            return null;
        try {
            const parsed = JSON.parse(last.content);
            if (parsed.agent !== 'code_agent')
                return null;
