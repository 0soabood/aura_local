...
exports.ReactiveOrchestrator = void 0;
...
function isSynthesisGreeting(text) {
    const t = text.toLowerCase();
    const hasIdentityAnchor = t.includes('what can you') || t.includes('who are you') || t.includes('how do you work');
    if (text.length <= 30)
        return SYNTHESIS_GREETING_KEYWORDS.some(k => t.includes(k));
    return hasIdentityAnchor;
}
function hasDomainWords(text) {
    const t = text.toLowerCase();
    ...
    async run(task) {
        ...
        while (loops < MAX_LOOPS) {
            ...
            if (last?.event_type === 'synthesis_complete' ||
                ...
                terminationReason = last.event_type;
                break;
            }
            ...
            if (aboveThreshold.length > 0) {
                ...
                const health = await this.providerHealth().catch(() => ({}));
                const anyHealthy = Object.values(health).some(Boolean);
                if (!anyHealthy) {
                    terminationReason = 'no_bid';
                    BlackboardEventRepository_1.BlackboardEventRepository.append(sessionId, 'escalation_required', 'orchestrator', JSON.stringify({
                        reason: 'All model providers are currently unavailable.',
                        actionable: 'Set at least one of GROQ_API_KEY, GOOGLE_AI_STUDIO_API_KEY, ' +
                ...
                if (isDomainQuery && bestSpecialist) {
                    ...
                    console.log(`[Orchestrator] loop=${loops + 1}  below-threshold specialist pick: ` +
                    ...
                    console.log(`[Orchestrator] loop=${loops + 1}  no specialist bid — synthesis conversational fallback.`);
                }
            }
            ...
            try {
                ...
                if (winner.agentName === 'synthesis_agent' &&
                ...
                if (output.event_type === 'synthesis_complete' ||
                    output.event_type === 'escalation_required') {
                    terminationReason = output.event_type;
                    break;
                }
            }
            catch (err) {
                console.error(`[Orchestrator] ${winner.agentName} execute() threw:`, err.message);
                BlackboardEventRepository_1.BlackboardEventRepository.append(sessionId, 'execution_error', 'orchestrator', JSON.stringify({
                    agent: winner.agentName,
                    error: err.message,
                    loop: loops + 1,
                    bid: winner,
    ...
    async callWithFallback(events, bid) {
        ...
        try {
            const result = await this.registry.callWithFallback(prompt, {
                temperature: 0.3,
                systemPrompt: (0, prompts_1.assembleSystemPrompt)(synthBase),
            });
            return {
            ...
            return {
                ...
                content: JSON.stringify({
                    ...
                    actionable: 'Check that at least one of GROQ_API_KEY, GOOGLE_AI_STUDIO_API_KEY, ' +
                        'OPENROUTER_API_KEY, MISTRAL_API_KEY, COHERE_API_KEY, or DEEPSEEK_API_KEY ' +
...
exports.ReactiveOrchestrator = ReactiveOrchestrator;
