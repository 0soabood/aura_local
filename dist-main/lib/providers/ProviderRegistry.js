...
const UnifiedCaller_1 = require("./UnifiedCaller");
...
class ProviderRegistry {
    ...
    recentCallCount(providerId, rpm) {
     ...
     */
    getAvailableProviders(preferred) {
     ...
     */
    getAvailableProvider(preferred) {
     ...
     */
    async call(routing, prompt, opts = {}) {
        ...
        const result = await (0, UnifiedCaller_1.callProvider)(modelId, messages, {
            format: cfg.format,
            baseUrl: cfg.baseUrl,
            apiKey,
            providerId,
     ...
     */
    async callWithFallback(prompt, opts = {}) {
        ...
        for (const cfg of providers) {
            ...
            try {
                const result = await (0, UnifiedCaller_1.callProvider)(cfg.defaultModel, messages, {
                    format: cfg.format,
                    baseUrl: cfg.baseUrl,
                    apiKey,
                    providerId: cfg.id,
                ...
                console.log(`[ProviderRegistry] Served by ${cfg.id}:${cfg.defaultModel} ` +
        ...
        throw new Error(`[ProviderRegistry] All providers failed: ${skipped.join(' | ')}`);
    }
    // ── Health check ──────────────────────────────────────────────────────────
    ...
    debugOutboundMessages(path, messages) {
        ...
        console.log(`[LLM DEBUG] path=${path} ` +
        ...
        systemMessages.forEach((m, i) => {
            console.log(`[LLM DEBUG] system[${i}] len=${m.content.length} ` +
                `first120=${JSON.stringify(m.content.slice(0, 120))}`);
        });
        if (multipleSystemChannels) {
            console.warn(`[LLM DEBUG] WARNING: ${path} has ${systemMessages.length} system messages — ` +
